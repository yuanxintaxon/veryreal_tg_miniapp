...
class CachedVideoControllerService extends VideoControllerService {
  ...
  Future<VideoPlayerController> getVideo(String videoUrl) async {
    ...
    if (file == null) {
      ...
      // FileInfo info = await _cacheManager.downloadFile(videoUrl);
      // Logger.print(
      //     '[VideoControllerService]: Saving video to cache ${info.file.path}');
      unawaited(_cacheManager.downloadFile(videoUrl));
      return VideoPlayerController.networkUrl(Uri.parse(videoUrl));
    } else {
      Logger.print('[VideoControllerService]: Loading video from cache $file');
...
class VeryVideoPlayerView extends StatefulWidget {
  const VeryVideoPlayerView({
    super.key,
    this.path,
    this.url,
    this.coverUrl,
  ...
  @override
  State<VeryVideoPlayerView> createState() => _VeryVideoPlayerViewState();
}

class _VeryVideoPlayerViewState extends State<VeryVideoPlayerView>
